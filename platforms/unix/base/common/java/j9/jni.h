...
struct JNINativeInterface_ {
	...
	jobject (PJNICALL GetObjectArrayElement)(JNIEnv *env, jobjectArray array, jsize index);
	void (PJNICALL SetObjectArrayElement)(JNIEnv *env, jobjectArray array, jsize index, jobject val);
...
struct JNIEnv_ {
	...
	void GetStringRegion(jstring str, jsize start, jsize len, char *buf) { functions->GetStringRegion(this, str, start, len, buf); }
	...
	void ReleaseStringCritical(jstring string, jchar *carray) { functions->ReleaseStringCritical(this, string, carray); }
