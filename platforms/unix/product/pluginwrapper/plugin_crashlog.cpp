...
namespace PluginCrashlog
{
	...
	const char* crashlog_parameters[] =
		{ NULL,
		  "-crashlog",
		  pid,
		  "-logfolder",
...
void PluginCrashlog::InstallHandler(const char* pathname, const char* logfolder)
{
	crashlog_parameters[0] = pathname;
	op_itoa(getpid(), pid, 10);
	crashlog_parameters[4] = logfolder;
