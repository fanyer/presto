...
#if !defined PC_MACOS_H && defined PREFS_HAVE_MAC
# define PC_MACOS_H

# include "modules/prefs/prefsmanager/opprefscollection_default.h"

/** Global PrefsCollectionMacOS object (singleton). */
# define g_pcmacos (g_opera->prefs_module.PrefsCollectionMacOS())

/**
  * Preferences for the Macintosh platform.
  *
  * @author Peter Karlsson
  */
class PrefsCollectionMacOS : public OpPrefsCollectionDefault
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionMacOS *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionMacOS();

# include "platforms/mac/prefs/PrefsCollectionMacOS_h.inl"
...
# ifdef PREFS_ENUMERATE
	...
	{
		return PCMACOS_NUMBEROFSTRINGPREFS + PCMACOS_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionMacOS(PrefsFile *reader)
		: OpPrefsCollectionDefault(MacOS, reader)
		{
...
# endif
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCMACOS_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCMACOS_NUMBEROFINTEGERPREFS + 1];
...
#endif // !PC_MACOS_H && PREFS_HAVE_MAC
