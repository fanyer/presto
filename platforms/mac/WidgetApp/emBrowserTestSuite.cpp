...
EmBrowserStatus TestCookiesRegrRerun()
{
	...
	// regression test for bugs #141105 and 140935
	...
	{ 	// go through existing cookies to check browser calls
		...
		TestError(((domain_cookies_count = g_methods->getCookieCount(testdomain1, NULL)) != 1), "Method returned value other than 1 (slash test 1)", "EmBrowserGetCookieCountProc");
		TestError(((domain_cookies_count = g_methods->getCookieCount(testdomain1, testpath2)) != 1), "Method returned value other than 1 (slash test 2)", "EmBrowserGetCookieCountProc");
