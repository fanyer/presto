...
/** Global PrefsCollectionGeolocation object (singleton). */
#define g_pcgeolocation (g_opera->prefs_module.PrefsCollectionGeolocation())

/**
  * Preferences related to the geolocation module
  *
  * @author Johannes Hoff
  */
class PrefsCollectionGeolocation : public OpPrefsCollectionDefault
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionGeolocation *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionGeolocation();

#include "modules/prefs/prefsmanager/collections/pc_geoloc_h.inl"
...
# endif
	...
	{
		return PCGEOLOCATION_NUMBEROFSTRINGPREFS + PCGEOLOCATION_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionGeolocation(PrefsFile *reader)
		: OpPrefsCollectionDefault(Geolocation, reader)
		{
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCGEOLOCATION_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCGEOLOCATION_NUMBEROFINTEGERPREFS + 1];
