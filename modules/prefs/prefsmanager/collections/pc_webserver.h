...
#if !defined PC_WEBSERVER_H && defined WEBSERVER_SUPPORT
#define PC_WEBSERVER_H

#include "modules/prefs/prefsmanager/opprefscollection_default.h"

/** Global PrefsCollectionWebserver object (singleton). */
#define g_pcwebserver (g_opera->prefs_module.PrefsCollectionWebserver())

/**
  * Preferences for the webserver.
  *
  * @author Peter Karlsson
  */
class PrefsCollectionWebserver : public OpPrefsCollectionDefault
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionWebserver *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionWebserver();

#include "modules/prefs/prefsmanager/collections/pc_webserver_h.inl"
...
# endif
	...
	{
		return PCWEBSERVER_NUMBEROFSTRINGPREFS + PCWEBSERVER_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionWebserver(PrefsFile *reader)
		: OpPrefsCollectionDefault(Webserver, reader)
		{
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCWEBSERVER_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCWEBSERVER_NUMBEROFINTEGERPREFS + 1];
...
#endif // !PC_WEBSERVER_H && WEBSERVER_SUPPORT
