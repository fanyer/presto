...
#define g_pcjs (g_opera->prefs_module.PrefsCollectionJS())

/**
  * Preferences for EcmaScript.
  *
  * @author Peter Karlsson
  */
class PrefsCollectionJS : public OpPrefsCollectionWithHostOverride
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionJS *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionJS();

#include "modules/prefs/prefsmanager/collections/pc_js_h.inl"
	 ...
	 * @param exact_match Match the host name exactly, i.e. without sub-domain matching.
	 */
	inline int GetIntegerPref(integerpref which, const uni_char *host = NULL, BOOL exact_match = FALSE) const
	{
		return OpPrefsCollectionWithHostOverride::GetIntegerPref(int(which), host, exact_match);
	}
...
# endif
	...
	{
		return PCJS_NUMBEROFSTRINGPREFS + PCJS_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionJS(PrefsFile *reader)
		: OpPrefsCollectionWithHostOverride(JS, reader)
		{
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCJS_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCJS_NUMBEROFINTEGERPREFS + 1];
