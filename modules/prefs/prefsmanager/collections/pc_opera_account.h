...
#if !defined PC_OPERA_ACCOUNT_H && defined PREFS_HAVE_OPERA_ACCOUNT
#define PC_OPERA_ACCOUNT_H

#include "modules/prefs/prefsmanager/opprefscollection_default.h"

/** Global PrefsCollectionOperaAccount object (singleton). */
#define g_pcopera_account (g_opera->prefs_module.PrefsCollectionOperaAccount())

/** Preferences for Opera Account. */
class PrefsCollectionOperaAccount : public OpPrefsCollectionDefault
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionOperaAccount *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionOperaAccount();

#include "modules/prefs/prefsmanager/collections/pc_opera_account_h.inl"
...
# endif
	...
	{
		return PCOPERA_ACCOUNT_NUMBEROFSTRINGPREFS + PCOPERA_ACCOUNT_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionOperaAccount(PrefsFile *reader)
		: OpPrefsCollectionDefault(OperaAccount, reader)
		{
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCOPERA_ACCOUNT_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCOPERA_ACCOUNT_NUMBEROFINTEGERPREFS + 1];
...
#endif // !PC_OPERA_ACCOUNT_H && PREFS_HAVE_OPERA_ACCOUNT
