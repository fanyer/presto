...
#if !defined PC_UNIX_H && defined PREFS_HAVE_UNIX
#define PC_UNIX_H

#include "modules/prefs/prefsmanager/opprefscollection_default.h"

/** Global PrefsCollectionUnix object (singleton). */
#define g_pcunix (g_opera->prefs_module.PrefsCollectionUnix())

/**
  * Preferences for the Unix platform(s).
  *
  * @author Peter Karlsson
  */
class PrefsCollectionUnix : public OpPrefsCollectionDefault
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionUnix *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionUnix();

#include "modules/prefs/prefsmanager/collections/pc_unix_h.inl"
...
# endif
	...
	{
		return PCUNIX_NUMBEROFSTRINGPREFS + PCUNIX_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionUnix(PrefsFile *reader)
		: OpPrefsCollectionDefault(Unix, reader)
		{
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCUNIX_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCUNIX_NUMBEROFINTEGERPREFS + 1];
...
#endif // !PC_UNIX_H && PREFS_HAVE_UNIX
