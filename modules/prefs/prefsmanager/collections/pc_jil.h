...
/** Global PrefsCollectionJIL object (singleton). */
#define g_pcjil (g_opera->prefs_module.PrefsCollectionJIL())

/**
  * Preferences for the JIL API.
  *
  * @author Wojciech Maslowski(wmaslowski@opera.com)
  */
class PrefsCollectionJIL : public OpPrefsCollectionDefault
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionJIL *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionJIL();

#include "modules/prefs/prefsmanager/collections/pc_jil_h.inl"
...
# endif
	...
	{
		return PCJIL_NUMBEROFSTRINGPREFS + PCJIL_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionJIL(PrefsFile *reader)
		: OpPrefsCollectionDefault(JIL, reader)
		{
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCJIL_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCJIL_NUMBEROFINTEGERPREFS + 1];
