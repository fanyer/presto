...
#define XML_SPACE           0x01
#define XML_NAMEFIRST       0x02
#define XML_NAME            0x04
#define XML_HEXDIGIT        0x08
#define XML_DECDIGIT        0x10
#define XML_VALID_10        0x20
#define XML_VALID_11        0x40
#define XML_UNRESTRICTED_11 0x80

/* static */ const unsigned char
XMLUtils::characters[128] = {
	
};

static const uni_char
XMLUtils_namefirst10[] = {
...
static const uni_char
XMLUtils_name10[] = {
	0x00B7, 0x00B7, 0x02D0, 0x02D1, 0x0300, 0x0345, 0x0360, 0x0361,
	...
	0x0D66, 0x0D6F, 0x0E31, 0x0E31, 0x0E34, 0x0E3A, 0x0E46, 0x0E4E,
	0x0E50, 0x0E59, 0x0EB1, 0x0EB1, 0x0EB4, 0x0EB9, 0x0EBB, 0x0EBC,
	0x0EC6, 0x0EC6, 0x0EC8, 0x0ECD, 0x0ED0, 0x0ED9, 0x0F18, 0x0F19,
	0x0F20, 0x0F29, 0x0F35, 0x0F35, 0x0F37, 0x0F37, 0x0F39, 0x0F39,
	0x0F3E, 0x0F3E, 0x0F3F, 0x0F3F, 0x0F71, 0x0F84, 0x0F86, 0x0F8B,
	0x0F90, 0x0F95, 0x0F97, 0x0F97, 0x0F99, 0x0FAD, 0x0FB1, 0x0FB7,
	0x0FB9, 0x0FB9, 0x20D0, 0x20DC, 0x20E1, 0x20E1, 0x3005, 0x3005,
	0x302A, 0x302F, 0x3031, 0x3035, 0x3099, 0x3099, 0x309A, 0x309A,
	0x309D, 0x309E, 0x30FC, 0x30FE
};

static const uni_char
XMLUtils_namefirst11[] = {
...
static BOOL
XMLUtils_FindChar(const uni_char *ranges, unsigned ranges_count, uni_char c)
{
	int low = 0, high = ranges_count - 2, index;
...
/* static */ BOOL
XMLUtils::IsSpace(unsigned ch)
{
	return ch < 128 && (characters[ch] & XML_SPACE) != 0;
}

/* static */ BOOL
XMLUtils::IsSpaceExtended(XMLVersion version, unsigned ch)
{
	return IsSpace(ch) || version == XMLVERSION_1_1 && (ch == 0x85 || ch == 0x2028);
}

static BOOL
XMLUtils_IsNameFirst10(unsigned ch)
{
	return XMLUtils_FindChar(XMLUtils_namefirst10, sizeof XMLUtils_namefirst10 / sizeof XMLUtils_namefirst10[0], ch);
}

static BOOL
XMLUtils_IsNameFirst11(unsigned ch)
{
	return XMLUtils_FindChar(XMLUtils_namefirst11, sizeof XMLUtils_namefirst11 / sizeof XMLUtils_namefirst11[0], ch);
}

/* static */ BOOL
XMLUtils::IsNameFirst(XMLVersion version, unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & XML_NAMEFIRST) != 0;
	else if (ch < 0x10000)
		return version == XMLVERSION_1_0 ? XMLUtils_IsNameFirst10(ch) : XMLUtils_IsNameFirst11(ch);
	else
		return version != XMLVERSION_1_0 && ch <= 0xeffff;
}

static BOOL
XMLUtils_IsName10(unsigned ch)
{
	return XMLUtils_IsNameFirst10(ch) || XMLUtils_FindChar(XMLUtils_name10, sizeof XMLUtils_name10 / sizeof XMLUtils_name10[0], ch);
}

static BOOL
XMLUtils_IsName11(unsigned ch)
{
	return XMLUtils_IsNameFirst11(ch) || ch == 0x00B7 || ch >= 0x0300 && ch <= 0x036F || ch >= 0x203F && ch <= 0x2040;
}

/* static */ BOOL
XMLUtils::IsName(XMLVersion version, unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & XML_NAME) != 0;
	else if (ch < 0x10000)
		return version == XMLVERSION_1_0 ? XMLUtils_IsName10(ch) : XMLUtils_IsName11(ch);
	else
		return FALSE;
}

/* static */ BOOL
XMLUtils::IsDecDigit(unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & XML_DECDIGIT) != 0;
	else
		return FALSE;
}

/* static */ BOOL
XMLUtils::IsHexDigit(unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & XML_HEXDIGIT) != 0;
	else
		return FALSE;
}

static BOOL
XMLUtils_IsCharHigh(unsigned ch)
{
	return !(ch >= 0xd800 && ch <= 0xdfff || ch == 0xfffe || ch == 0xffff || ch > 0x10ffff);
}

/* static */ BOOL
XMLUtils::IsChar10(unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & XML_VALID_10) != 0;
	else
		return XMLUtils_IsCharHigh(ch);
}

/* static */ BOOL
XMLUtils::IsChar11(unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & XML_VALID_11) != 0;
	else
		return XMLUtils_IsCharHigh(ch);
}

/* static */ BOOL
XMLUtils::IsChar(XMLVersion version, unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & (version == XMLVERSION_1_0 ? XML_VALID_10 : XML_VALID_11)) != 0;
	else
		return XMLUtils_IsCharHigh(ch);
}

/* static */ BOOL
XMLUtils::IsRestrictedChar(unsigned ch)
{
	if (ch < 128)
		return (characters[ch] & XML_UNRESTRICTED_11) == 0;
	else
		return ch >= 127 && ch <= 159 && ch != 133;
}

/* static */ unsigned
XMLUtils::GetNextCharacter(const uni_char *&string, unsigned &string_length)
{
	unsigned ch = *string;
	...
	if (ch >= 0xd800 && ch <= 0xdbff && string_length != 0)
	{
		unsigned ch2 = *string;
...
/* static */ BOOL
XMLUtils::IsValidName(XMLVersion version, const uni_char *name, unsigned name_length)
{
	if (name_length == ~0u)
		name_length = uni_strlen(name);

	if (name_length == 0 || !IsNameFirst(version, XMLUtils::GetNextCharacter(name, name_length)))
		return FALSE;
...
/* static */ BOOL
XMLUtils::IsValidNCName(XMLVersion version, const uni_char *name, unsigned name_length)
{
	if (name_length == ~0u)
		name_length = uni_strlen(name);
...
/* static */ BOOL
XMLUtils::IsWhitespace(const uni_char *string, unsigned string_length)
{
	if (string_length == ~0u)
		string_length = uni_strlen(string);

	while (string_length != 0)
		if (!IsSpace(XMLUtils::GetNextCharacter(string, string_length)))
			return FALSE;
...
/* static */ BOOL
XMLUtils::GetLine(XMLVersion version, const uni_char *&data, unsigned &data_length, const uni_char *&line, unsigned &line_length)
{
	if (data_length != 0)
	{
		BOOL is_xml_1_1 = version == XMLVERSION_1_1;
		const uni_char *ptr = data, *ptr_end = data + data_length;
...
XmlutilsModule::XmlutilsModule()
	: OperaModule()
#ifdef XML_CONFIGURABLE_DOCTYPES
	, configureddoctypes(0)
#endif // XML_CONFIGURABLE_DOCTYPES
{
}

void
XmlutilsModule::InitL(const OperaInitInfo& info)
{
#if defined XML_CONFIGURABLE_DOCTYPES
	/* FIXME: OP_NEW_L() doesn't work here, because Head -- base class of
	   XMLConfiguredDoctypes -- overloads operator new. */
	configureddoctypes = OP_NEW_L (XMLConfiguredDoctypes, ());
#endif // XML_CONFIGURABLE_DOCTYPES
}

void
XmlutilsModule::Destroy()
{
#if defined XML_CONFIGURABLE_DOCTYPES
	/* FIXME: Not using OP_DELETE simply because we didn't use OP_NEW* above. */
	OP_DELETE(configureddoctypes);
#endif // XML_CONFIGURABLE_DOCTYPES
}
