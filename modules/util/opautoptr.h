...
private:
	...
	/**
	 * Can be overridden by template specialization
	 * to delete the object in a different way.
	 *
	 * Example:
	 *   template<> inline void OpAutoPtr<MyClass>::DeletePtr(MyClass *p) { MyClass::BeGone(p); }
	 *
	 * Note: put the template specialization in a
	 * header and don't forget the inline keyword.
	 *
	 * @param p   The pointer to be disposed. Never NULL.
	 *
	 */
	inline void DeletePtr(T* p) { OP_DELETE(p); }
