...
{
private:
	// Head::Append() need direct access to private members of the Link class
	friend class Head;
...
public:
	...
	/** Transfer all items in given list to this list. */
	void			Append(Head *list);

	/** Check if element belongs to this list. */
	BOOL			HasLink(const Link* element) const { return element->GetList() == this; }

	/** Comparison function for Sort().
	 *  Must return something > 0 if lhs > rhs,
	 *  something < 0 if lhs < rhs else 0.
	 **/
	typedef			int (*LinkSortFn)(const Link* lhs, const Link* rhs);

	/** Sorts the list in ascending order given the comparison function. */
	void			Sort(LinkSortFn comparator);
...
/**
 * Thin template wrapper for HashedLink, eliminating the need for typecasts.
 * When subclassing, use the subclass itself as the element type template
 * parameter E. For instance:
 *
 * <pre>
 * class A: public HashedListElement<A>
 * {
 *     // ... payload data
 * };
 *
 ...
 *
 * If you chose private inheritance, where the above uses public, you'll need a
 * friend class declaration in the body of class A to avoid errors.
 */
template<class E>
class HashedListElement : public HashedLink // Should use private inheritance, but VS6 sucks
{
	friend class List<E>;
protected:
	List<E>* GetList() const				{ return static_cast<List<E>*>(HashedLink::GetList()); }
public:
	E* Suc() const							{ return static_cast<E*>(HashedLink::Suc()); }
	E* Pred() const							{ return static_cast<E*>(HashedLink::Pred()); }
	void Into(List<E>* list)				{ HashedLink::Into(list); }
	void IntoStart(List<E>* list)			{ HashedLink::IntoStart(list); }
	void Follow(E* element)					{ HashedLink::Follow(element); }
	void Precede(E* element)				{ HashedLink::Precede(element); }
	BOOL Precedes(const E* element) const	{ return HashedLink::Precedes(element); }
};
...
public:
	...
	BOOL HasLink(const E* element) const	{ return Head::HasLink(element); }

	/** Comparison function for Sort().
	 *  Must return something > 0 if lhs > rhs,
	 *  something < 0 if lhs < rhs else 0.
	 **/
	typedef			int (*ListElmSortFn)(const E* lhs, const E* rhs);

	/** Sorts the list in ascending order given the comparison function. */
	void			Sort(ListElmSortFn comparator) { Head::Sort(reinterpret_cast<LinkSortFn>(comparator)); }
};
