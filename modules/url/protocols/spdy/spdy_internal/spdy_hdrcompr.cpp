...
const char Spdy2CompressionDictionary[] =
...
const int Spdy2CompressionDictionarySize = ARRAY_SIZE(Spdy2CompressionDictionary);

const char Spdy3CompressionDictionary[] = {
...
const int Spdy3CompressionDictionarySize = ARRAY_SIZE(Spdy3CompressionDictionary);

SpdyCompressionProvider::SpdyCompressionProvider(SpdyVersion ver, BOOL secureMode): version(ver), secureMode(secureMode)
{
	op_memset(&compressor, 0, sizeof(z_stream));
	op_memset(&decompressor, 0, sizeof(z_stream));
}

OP_STATUS SpdyCompressionProvider::Init()
{
	const int compressorLevel = 9;
	const int compressorWindowSizeInBits = 11;
	const int compressorMemLevel = 1;

	int result;
	if((result = deflateInit2(&compressor, compressorLevel, Z_DEFLATED, compressorWindowSizeInBits, compressorMemLevel, Z_DEFAULT_STRATEGY)) != Z_OK)
		return ZlibErrorToOpStatus(result);

	const char *compressionDictionary = version == SPDY_V2 ? Spdy2CompressionDictionary : Spdy3CompressionDictionary;
	const int compressionDictionarySize = version == SPDY_V2 ? Spdy2CompressionDictionarySize : Spdy3CompressionDictionarySize;
	if((result = deflateSetDictionary(&compressor, reinterpret_cast<const Bytef*>(compressionDictionary), compressionDictionarySize)) != Z_OK)
		return ZlibErrorToOpStatus(result);

	if((result = inflateInit(&decompressor)) != Z_OK)
		return ZlibErrorToOpStatus(result);

	return OpStatus::OK;
}

SpdyCompressionProvider::~SpdyCompressionProvider()
{
	inflateEnd(&decompressor);
	deflateEnd(&compressor);
}

OP_STATUS SpdyCompressionProvider::Decompress()
{
	...
	if (res == Z_NEED_DICT) 
	{
		const char *compressionDictionary = version == SPDY_V2 ? Spdy2CompressionDictionary : Spdy3CompressionDictionary;
		const int compressionDictionarySize = version == SPDY_V2 ? Spdy2CompressionDictionarySize : Spdy3CompressionDictionarySize;
		if (res = inflateSetDictionary(&decompressor, reinterpret_cast<const Bytef*>(compressionDictionary), compressionDictionarySize) != Z_OK)
			return ZlibErrorToOpStatus(res);
		res = inflate(&decompressor, Z_SYNC_FLUSH);
	}
	if (res == Z_BUF_ERROR && (decompressor.avail_in == 0 || decompressor.avail_out == 0))
		return OpStatus::OK;
	return ZlibErrorToOpStatus(res);
}

OP_STATUS SpdyCompressionProvider::Compress(int flushType)
{
	int res = deflate(&compressor, flushType);
	return ZlibErrorToOpStatus(res);
}

OP_STATUS SpdyCompressionProvider::ZlibErrorToOpStatus(int error)
{
	switch (error)
	{
	case Z_OK:
...
z_stream& SpdyCompressionProvider::GetCompressionStream()
{
	return compressor;
}

z_stream& SpdyCompressionProvider::GetDecompressionStream()
{
	return decompressor;
}
