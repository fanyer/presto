...
#include "modules/webp/lib/enc/histogram.h"
...
float VP8LFastLog2(int v) {
  if (v < LOG_LOOKUP_IDX_MAX) {
    ...
    return (float)(LOG_2_RECIPROCAL * vp8_log((double)v));
  }
}
...
static float PredictionCostSpatialHistogram(int accumulated[4][256],
  ...
  int i;
  int k;
  int combo[256]; // ARRAY OK 2012-08-22 andersr
  double retval = 0;
  for (i = 0; i < 4; ++i) {
...
static int GetBestPredictorForTile(int width, int height,
  ...
  int histo[4][256]; // ARRAY-OK 2012-08-22 andersr
...
static Multipliers GetBestColorTransformForTile(
  ...
  for (green_to_red = -64; green_to_red <= 64; green_to_red += halfstep) {
    int histo[256] = { 0 }; // ARRAY OK 2012-08-22 andersr
    int all_y;
    Multipliers tx;
    MultipliersClear(&tx);
...
void VP8LColorSpaceTransform(int width, int height, int bits, int step,
  ...
  int accumulated_red_histo[256] = { 0 }; // ARRAY OK 2012-08-22 andersr
  int accumulated_blue_histo[256] = { 0 }; // ARRAY OK 2012-08-22 andersr
  int tile_y;
  int tile_x;
  Multipliers prevX;
  Multipliers prevY;
...
static int is_big_endian(void) {
  static const union {
    uint16_t w;
    uint8_t b[2]; // ARRAY OK 2012-08-22 andersr
  } tmp = { 1 };
  return (tmp.b[0] != 1);
}
