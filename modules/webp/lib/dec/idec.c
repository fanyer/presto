...
typedef struct {
  VP8MB left_;
  VP8MB info_;
  uint8_t intra_t_[4]; // ARRAY OK 2012-08-22 andersr
  uint8_t intra_l_[4]; // ARRAY OK 2012-08-22 andersr
  VP8BitReader br_;
...
static int CopyParts0Data(WebPIDecoder* const idec) {
  ...
  if (mem->mode_ == MEM_MODE_APPEND) {
    ...
    uint8_t* const part0_buf = (uint8_t*)vp8_malloc(psize);
    if (part0_buf == NULL) {
      return 0;
    }
...
static VP8StatusCode DecodeVP8LHeader(WebPIDecoder* const idec) {
  ...
  if (!VP8LDecodeHeader(dec, io)) {
    return (VP8StatusCode)ErrorStatusLossless(idec, dec->status_);
  }
  // Allocate/verify output buffer now.
  dec->status_ = WebPAllocateDecBuffer(io->width, io->height, params->options,
                                       output);
...
WebPIDecoder* WebPINewDecoder(WebPDecBuffer* output_buffer) {
  WebPIDecoder* idec = (WebPIDecoder*)vp8_calloc(1, sizeof(*idec));
  if (idec == NULL) {
    return NULL;
  }
