...
#include "modules/webp/lib/utils/filters.h"
...
static int DecodeAlpha(const uint8_t* data, size_t data_size,
  ...
  filter = (WEBP_FILTER_TYPE)((data[0] >> 2) & 0x03);
  ...
  if (method == ALPHA_NO_COMPRESSION) {
    ...
    decoded_data = (uint8_t*)vp8_malloc(decoded_size);
  ...
  if (ok) {
    ...
    if (unfilter_func != NULL) {
      unfiltered_data = (uint8_t*)vp8_malloc(decoded_size);
