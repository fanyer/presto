...
#include "modules/webp/lib/utils/bit_reader.h"
#include "modules/webp/lib/utils/thread.h"
#include "modules/webp/lib/dsp/dsp.h"
...
typedef struct {
  ...
  int8_t quantizer_[NUM_MB_SEGMENTS];        // quantization changes // ARRAY OK 2012-08-22 andersr
  int8_t filter_strength_[NUM_MB_SEGMENTS];  // filter strength for segments // ARRAY OK 2012-08-22 andersr
} VP8SegmentHeader;
...
typedef struct {
  ...
  int sharpness_;               // [0..7]
  int use_lf_delta_;
  int ref_lf_delta_[NUM_REF_LF_DELTAS]; // ARRAY OK 2012-08-22 andersr
  int mode_lf_delta_[NUM_MODE_LF_DELTAS]; // ARRAY OK 2012-08-22 andersr
} VP8FilterHeader;
...
typedef int quant_t[2];      // [DC / AC].  Can be 'uint16_t[2]' too (~slower). // ARRAY OK 2012-08-22 andersr
typedef struct {
...
struct VP8Decoder {
  ...
  VP8BitReader parts_[MAX_NUM_PARTITIONS]; // ARRAY OK 2012-08-22 andersr
