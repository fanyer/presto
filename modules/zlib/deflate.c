...
/* WONKO: brew compiler can't cope with complex globals - the
 * configuration_table struct has been turned into an OperaGlobal.
 * these definitions have been moved to deflate_config.h
 */
...
#endif
#endif
local uInt longest_match_fast OF((deflate_state *s, IPos cur_match, int clas));
...
/* WONKO: brew compiler can't cope with complex globals - the
 * configuration_table struct has been turned into an OperaGlobal.
 * definition has been moved to deflate_config.h.
 */
#define ZLIB_SET_CONFIG_ENTRY(c, g, ml, n, mc, f) do {    \
    c.good_length = g;                          \
    c.max_lazy = g;                             \
    c.nice_length = g;                          \
    c.max_chain = g;                            \
...
{
#ifdef FASTEST
    ZLIB_SET_CONFIG_ENTRY(conf[0], 0,    0,  0,    0, deflate_stored);  /* store only */
    ZLIB_SET_CONFIG_ENTRY(conf[1], 4,    4,  8,    4, deflate_fast); /* max speed, no lazy matches */
#else // FASTEST
# ifndef ZLIB_REDUCE_FOOTPRINT
/*      good lazy nice chain */
    ZLIB_SET_CONFIG_ENTRY(conf[0], 0,    0,  0,    0, deflate_stored);  /* store only */
    ZLIB_SET_CONFIG_ENTRY(conf[1], 4,    4,  8,    4, deflate_fast); /* max speed, no lazy matches */
    ZLIB_SET_CONFIG_ENTRY(conf[2], 4,    5, 16,    8, deflate_fast);
    ZLIB_SET_CONFIG_ENTRY(conf[3], 4,    6, 32,   32, deflate_fast);
# else // ZLIB_REDUCE_FOOTPRINT
/*      good lazy nice chain */
    ZLIB_SET_CONFIG_ENTRY(conf[0], 4,    4, 16,   16, deflate_slow);  /* store only */
    ZLIB_SET_CONFIG_ENTRY(conf[1], 4,    4, 16,   16, deflate_slow); /* max speed, no lazy matches */
    ZLIB_SET_CONFIG_ENTRY(conf[2], 4,    4, 16,   16, deflate_slow);
    ZLIB_SET_CONFIG_ENTRY(conf[3], 4,    4, 16,   16, deflate_slow);
# endif // ZLIB_REDUCE_FOOTPRINT
    ZLIB_SET_CONFIG_ENTRY(conf[4], 4,    4, 16,   16, deflate_slow);  /* lazy matches */
    ZLIB_SET_CONFIG_ENTRY(conf[5], 8,   16, 32,   32, deflate_slow);
    ZLIB_SET_CONFIG_ENTRY(conf[6], 8,   16, 128, 128, deflate_slow);
    ZLIB_SET_CONFIG_ENTRY(conf[7], 8,   32, 128, 256, deflate_slow);
    ZLIB_SET_CONFIG_ENTRY(conf[8], 32, 128, 258, 1024, deflate_slow);
    ZLIB_SET_CONFIG_ENTRY(conf[9], 32, 258, 258, 4096, deflate_slow); /* max compression */
#endif // FASTEST
}
#if 0
...
# else // ZLIB_REDUCE_FOOTPRINT
/*      good lazy nice chain */
/* 0 */ {4,    4, 16,   16, deflate_slow},  /* store only */
/* 1 */ {4,    4, 16,   16, deflate_slow}, /* max speed, no lazy matches */
...
int ZEXPORT deflateParams(z_streamp strm, int level, int strategy)
{
    ...
    compress_func func;
    int err = Z_OK;
    config* configuration_table = get_deflate_config();
...
int ZEXPORT deflate (z_streamp strm, int flush)
{
...
#if defined GZIP && !defined ZLIB_REDUCE_FOOTPRINT
...
#endif // GZIP && !ZLIB_REDUCE_FOOTPRINT
    ...
    if (strm->avail_in != 0 || s->lookahead != 0 ||
        ...
        bstate = (*(configuration_table[s->level].func))(s, flush, strm->clas);
...
local void class_set(deflate_state *s, IPos offset, uInt len, int clas)
{
    IPos byte = offset >> 3;
    IPos bit = offset & 7;
...
local int class_at(deflate_state *s, IPos window_offset)
{
 ...
 */
local uInt longest_match(deflate_state *s, IPos cur_match, int clas)
...
local uInt cookie_match(deflate_state *s, IPos start, unsigned len)
{
    unsigned hash = 5381;
 ...
 */
local uInt longest_match_fast(deflate_state *s, IPos cur_match, int clas)
/* cur_match - current match */
{
    register Bytef *scan = s->window + s->strstart; /* current string */
    register Bytef *match;                       /* matched string */
...
static void FLUSH_BLOCK_ONLY(deflate_state *s, int eof)
{
 ...
 */
local block_state deflate_stored(deflate_state *s, int flush, int clas)
{
 ...
 */
local block_state deflate_fast(deflate_state *s, int flush, int clas)
{
    ...
    if (clas != 0) {
        ...
        return (block_state)Z_BUF_ERROR;
    }
    ...
    for (;;) {
        ...
        if (hash_head != NIL && s->strstart - hash_head <= MAX_DIST(s)) {
            ...
            if (s->strategy != Z_HUFFMAN_ONLY && s->strategy != Z_RLE) {
                ...
                s->match_length = longest_match_fast (s, hash_head, clas);
            }
#endif
 ...
 */
local block_state deflate_slow(deflate_state *s, int flush, int clas)
{
