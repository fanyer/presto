...
#include "modules/zlib/inftrees.h"
#include "modules/zlib/inflate.h"
#include "modules/zlib/inffast.h"
...
int ZEXPORT inflate(z_streamp strm, int flush)
{
    ...
    int ret;                    /* return code */
#ifdef GUNZIP
    unsigned char hbuf[4]; /* ARRAY OK 2008-04-17 wonko */      /* buffer for gzip header crc calculation */
#endif
    ...
    for (;;)
        ...
        case MATCH:
            ...
            if (state->offset > copy) {         /* copy from window */
                from = NULL; /* bug #184931 */
                copy = state->offset - copy;
                if (copy > state->write) {
                    copy -= state->write;
                    if (state->window) /* bug #184931 */
                        from = state->window + (state->wsize - copy);
                }
                else if (state->window) /* bug #184931 */
            ...
            /* bug #184931 */
            if (!from || state->offset > state->whave + out - left) {
                do {
                    *put++ = 0;
                } while (--copy);
            }
...
int ZEXPORT inflateSync(z_streamp strm)
{
    ...
    unsigned char buf[4]; /* ARRAY OK 2008-04-17 wonko */       /* to restore bit buffer to byte string */
    struct inflate_state FAR *state;
    ...
    if (state->mode != SYNC) {
        ...
        // WONKO: don't want to overflow buf
        if (state->bits >= 5*8)
            return Z_DATA_ERROR;
...
int ZEXPORT inflateCopy(z_streamp dest, z_streamp source)
{
    ...
    /* check input */
    if (dest == Z_NULL || source == Z_NULL || source->state == Z_NULL) return Z_STREAM_ERROR;
