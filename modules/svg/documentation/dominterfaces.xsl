...
<xsl:template match="/">
  <html xmlns="http://www.w3.org/1999/xhtml">
	...
	<xsl:for-each select="supportdocument/interfacelist/interface">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
	    ...
	    <td>
	      <xsl:if test="@selftest = 'yes'">
		<xsl:attribute name="class">selftest</xsl:attribute>
                Yes
	      </xsl:if>
	      <xsl:if test="@selftest = 'no'">
		<xsl:attribute name="class">noselftest</xsl:attribute>
                No
	      </xsl:if>
	    </td>
	  </tr>
