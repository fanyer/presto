...
const char SHA512_version[]="SHA-512" OPENSSL_VERSION_PTEXT; /* ARRAY OK 2008-02-27 haavardm */
#endif
...
int SHA512_Final (unsigned char *md, SHA512_CTX *c)
	...
	n++;
	if (n > (sizeof(c->u)-16))
		op_memset (p+n,0,sizeof(c->u)-n), n=0,
		sha512_block_data_order (c,p,1);

	op_memset (p+n,0,sizeof(c->u)-16-n);
#ifdef	B_ENDIAN
...
int SHA512_Update (SHA512_CTX *c, const void *_data, size_t len)
	...
	if (c->num != 0)
		...
		if (len < n)
			...
			op_memcpy (p+c->num,data,n), c->num = 0;
			len-=n, data+=n;
			sha512_block_data_order (c,p,1);
			}
		}
	...
	if (len != 0)	op_memcpy (p,data,len), c->num = (int)len;
...
unsigned char *SHA384(const unsigned char *d, size_t n, unsigned char *md)
	{
	SHA512_CTX c;
	static unsigned char m[SHA384_DIGEST_LENGTH]; /* ARRAY OK UNUSED 2009-05-25 yngve */
...
unsigned char *SHA512(const unsigned char *d, size_t n, unsigned char *md)
	{
	SHA512_CTX c;
	static unsigned char m[SHA512_DIGEST_LENGTH]; /* ARRAY OK UNUSED 2009-05-25 yngve */
