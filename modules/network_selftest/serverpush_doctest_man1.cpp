...
ServerPush_DocBlockedTester::ServerPush_DocBlockedTester(URL &a_url, URL &ref, ServerPush_Filename_List &names)
...
ServerPush_DocBlockedTester::~ServerPush_DocBlockedTester()
{
	started = FALSE;
	current_item = NULL;
...
BOOL ServerPush_DocBlockedTester::Verify_function(URL_DocSelfTest_Event event, Str::LocaleString status_code)
{
	switch(event)
	{
	case URLDocST_Event_LoadingRestarted:
		if(!header_loaded)
		{
			ReportFailure("No header loaded before bodypart #%d", count+1);
			return FALSE;
		}
...
BOOL ServerPush_DocBlockedTester::CheckContent(BOOL finished)
{
	...
	if(!current_file && !current_descriptor)
	{
		...
		if(current_descriptor == NULL)
		{
			...
			{
				ReportFailure("Compare failed #%d (A).", current_item->GetItem());
				return FALSE;				
			}
			OP_DELETE(current_file);
			current_file = NULL;
	...
	do{
		...
		{
			ReportFailure("Compare failed #%d (B).", current_item->GetItem());
			return FALSE;				
		}
		...
		if(current_file->MoreData())
		{
			ReportFailure("Compare failed #%d (C).", current_item->GetItem());
			return FALSE;				
		}
