...
int main(int argc, char** argv)
{
  ...
  char *nword, *wf, *af;
  char as[(MAX_PREFIXES + MAX_SUFFIXES)]; // ARRAY OK 2009-04-23 jb (NOT CHECKED)
  char * ap;
...
void encodeit(struct affent * ptr, char * cs)
{
  ...
  int nm;
  int i, j, k;
  unsigned char mbr[MAX_WD_LEN]; // ARRAY OK 2009-04-23 jb (NOT CHECKED)
...
void pfx_chk (const char * word, int len, struct affent* ep, int num)
{
    ...
    unsigned char *	cp;		
    int			i;
    char	        tword[MAX_WD_LEN]; // ARRAY OK 2009-04-23 jb (NOT CHECKED)
...
{
  char * ap;
  char ts[MAX_LN_LEN]; // ARRAY OK 2009-04-23 jb (NOT CHECKED)
