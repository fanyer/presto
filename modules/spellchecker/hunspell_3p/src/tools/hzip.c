...
int get_freqdata(struct item *** dest, FILE * f, unsigned short * termword) {
    int freq[CODELEN]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    int i, j, k, n;
    union {
        char c[2]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
        unsigned short word;
    } u;
...
int encode_file(char ** table, int n, FILE *f, FILE *f2, unsigned short tw, char * key) {
    char bitbuf[BUFSIZE]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    int i, bits = 0;
    unsigned char cl, ch;
    int cx[2]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    union {
        char c[2]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
        unsigned short word;
    } u;
...
int prefixcompress(FILE *f, FILE *tempfile) {
    char buf[BUFSIZE]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    char buf2[BUFSIZE * 2];  // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    char prev[BUFSIZE];  // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    int prevlen = 0;
...
int hzip(const char * filename, char * key) {
    struct item ** list;
    char * table[CODELEN + 1];  // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    int n;
    char out[BUFSIZE];  // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    FILE *f, *f2, *tempfile;
