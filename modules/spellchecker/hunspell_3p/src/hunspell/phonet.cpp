...
#include "modules/spellchecker/hunspell_3p/src/hunspell/phonet.h"
...
static int myisalpha(char ch) {
  if ((unsigned char) ch < 128) return op_isalpha(ch);
  return 1;
}
...
int phonet (const char * inword, char * target,
              int len,
            phonetable & parms, int target_size)
  {
    ...
    char word[MAXPHONETUTF8LEN + 1]; // ARRAY OK 2009-04-23 jb
    if (len == -1) len = op_strlen(inword);
    if (len > MAXPHONETUTF8LEN) return 0;
    op_strcpy(word, inword);
      ...
      if (n >= 0) {
        ...
        while (parms.rules[n][0] == c) {
          ...
          while (*s != '\0'  &&  word[i+k] == *s
                 &&  !op_isdigit((unsigned char) *s)  &&  op_strchr("(-<^$", *s) == NULL) {
            k++;
            s++;
          }
          if (*s == '(') {
...
//            if (parms.followup  &&  k > 1  &&  n0 >= 0
            ...
            p0 = (parms.rules[n][0] != '\0'
                 &&  op_strchr(parms.rules[n]+1,'<') != NULL) ? 1:0;
            if (p0 == 1 &&  z == 0) {
              /**  rule with '<' is used  **/
              if (j > 0  &&  *s != '\0'
              ...
              if (parms.rules[n][0] != '\0'
                 &&  op_strstr(parms.rules[n]+1, "^^") != NULL) {
                if (c != '\0') {
                  target[j] = c;
                  j++;
				  if (j >= (int) target_size)
