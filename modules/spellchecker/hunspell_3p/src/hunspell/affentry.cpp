...
#include "modules/spellchecker/hunspell_3p/src/hunspell/affentry.h"
...
PfxEntry::PfxEntry(AffixMgr* pmgr, affentry* dp)
{
  ...
  if (opts & aeLONGCOND) {
    op_memcpy(c.conds, dp->c.l.conds1, MAXCONDLEN_1);
    c.l.conds2 = dp->c.l.conds2;
  } else op_memcpy(c.conds, dp->c.conds, MAXCONDLEN);
  next = NULL;
  nextne = NULL;
  nexteq = NULL;
...
{
    aflag = 0;
    if (appnd) op_free(appnd);
    if (strip) op_free(strip);
    pmyMgr = NULL;
    appnd = NULL;
    strip = NULL;
    if (opts & aeLONGCOND) op_free(c.l.conds2);
    if (morphcode && !(opts & aeALIASM)) op_free(morphcode);
    if (contclass && !(opts & aeALIASF)) op_free(contclass);
}

// add prefix to this word assuming conditions hold
char * PfxEntry::add(const char * word, int len)
{
    char tword[MAXWORDUTF8LEN + 4]; // ARRAY OK 2009-04-23 jb

    if ((len > stripl || (len == 0 && pmyMgr->get_fullstrip())) && 
       (len >= numconds) && test_condition(word) &&
       (!stripl || (op_strncmp(word, strip, stripl) == 0)) &&
       ((MAXWORDUTF8LEN + 4) > (len + appndl - stripl))) {
    /* we have a match so add prefix */
              char * pp = tword;
              if (appndl) {
                  op_strcpy(tword,appnd);
                  pp += appndl;
               }
               op_strcpy(pp, (word + stripl));
...
struct hentry * PfxEntry::checkword(const char * word, int len, char in_compound, const FLAG needflag)
{
    ...
    char                tmpword[MAXWORDUTF8LEN + 4]; // ARRAY OK 2009-04-23 jb
    ...
    // to meet the number of characters conditions, then test it
	 ...
	 if ((size_t) (tmpl + stripl) >= sizeof(tmpword) )
		 return NULL;
     ...
     if (tmpl > 0 || (tmpl == 0 && pmyMgr->get_fullstrip())) {
            ...
            if (stripl) op_strcpy(tmpword, strip);
            op_strcpy((tmpword + stripl), (word + appndl));
...
{
    int                 tmpl;   // length of tmpword
    char                tmpword[MAXWORDUTF8LEN + 4]; // ARRAY OK 2009-04-23 jb (NOT CHECKED)
...
char * PfxEntry::check_morph(const char * word, int len, char in_compound, const FLAG needflag)
{
    int                 tmpl;   // length of tmpword
    struct hentry *     he;     // hash entry of root word or NULL
    char                tmpword[MAXWORDUTF8LEN + 4]; // ARRAY OK 2009-02-09 jb (NOT CHECKED)
...
SfxEntry::SfxEntry(AffixMgr * pmgr, affentry* dp)
{
  ...
  // then copy over all of the conditions
  if (opts & aeLONGCOND) {
    op_memcpy(c.l.conds1, dp->c.l.conds1, MAXCONDLEN_1);
    c.l.conds2 = dp->c.l.conds2;
  } else op_memcpy(c.conds, dp->c.conds, MAXCONDLEN);
...
char * SfxEntry::add(const char * word, int len)
{
	...
	if ((size_t) len >= sizeof(tword))
		return NULL;

     /* make sure all conditions match */
     if ((len > stripl || (len == 0 && pmyMgr->get_fullstrip())) &&
        (len >= numconds) && test_condition(word + len, word) &&
        (!stripl || (op_strcmp(word + len - stripl, strip) == 0)) &&
        ((MAXWORDUTF8LEN + 4) > (len + appndl - stripl))) {
              /* we have a match so add suffix */
              op_strcpy(tword,word);
              if (appndl) {
                  op_strcpy(tword + len - stripl, appnd);
              } else {
                  *(tword + len - stripl) = '\0';
              }
              return mystrdup(tword);
    ...
    const FLAG badflag)
{
	...
	if ((size_t) (tmpl + stripl) >= sizeof(tmpword) || (size_t) len >= sizeof(tmpword) )
		 return NULL;
    ...
    if ((tmpl > 0 || (tmpl == 0 && pmyMgr->get_fullstrip())) &&
        (tmpl + stripl >= numconds)) {
            ...
            op_strcpy(tmpword, word);
            cp = (unsigned char *)(tmpword + tmpl);
            if (stripl) {
                op_strcpy((char *)cp, strip);
                tmpl += stripl;
                cp = (unsigned char *)(tmpword + tmpl);
            } else *cp = '\0';
            ...
            if (test_condition((char *) cp, (char *) tmpword)) {
                ...
                // store resulting root in wlst
                    ...
                    int cwrd = 1;
                    for (int k=0; k < *ns; k++)
                        if (op_strcmp(tmpword, wlst[k]) == 0) cwrd = 0;
                    if (cwrd) {
...
struct affentry
{
   ...
   char   conds[SETSIZE];   // ARRAY OK 2009-02-09 jb (NOT CHECKED) // array which encodes the conditions to be met
};
