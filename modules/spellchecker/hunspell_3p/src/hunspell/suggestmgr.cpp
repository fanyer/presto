...
#include "modules/spellchecker/hunspell_3p/src/hunspell/suggestmgr.h"
#include "modules/spellchecker/hunspell_3p/src/hunspell/htypes.h"
                       ...
                       AffixMgr * aptr)
{
  ...
  if (ckey) {  
    if (utf8) {
        w_char t[MAXSWL]; // ARRAY OK 2009-02-09 jb
        ckeyl = u8_u16(t, MAXSWL, ckey);
        ckey_utf = (w_char *) op_malloc(ckeyl * sizeof(w_char));
        if (ckey_utf) op_memcpy(ckey_utf, t, ckeyl * sizeof(w_char));
        else ckeyl = 0;
    } else {
        ckeyl = op_strlen(ckey);
    }
  }
  
  if (tryme) {  
    ctry = mystrdup(tryme);
    if (ctry) ctryl = op_strlen(ctry);
    if (ctry && utf8) {
        w_char t[MAXSWL]; // ARRAY OK 2009-02-09 jb
        ctryl = u8_u16(t, MAXSWL, tryme);
        ctry_utf = (w_char *) op_malloc(ctryl * sizeof(w_char));
        if (ctry_utf) op_memcpy(ctry_utf, t, ctryl * sizeof(w_char));
        else ctryl = 0;
    }
  }
}
...
{
  pAMgr = NULL;
  if (ckey) op_free(ckey);
  ckey = NULL;
  if (ckey_utf) op_free(ckey_utf);
  ckey_utf = NULL;
  ckeyl = 0;
  if (ctry) op_free(ctry);
  ctry = NULL;
  if (ctry_utf) op_free(ctry_utf);
  ctry_utf = NULL;
  ctryl = 0;
  maxSug = 0;
...
int SuggestMgr::testsug(char** wlst, const char * candidate, int wl, int ns, int cpdsuggest,
   int * timer, clock_t * timelimit) {
      ...
      for (int k=0; k < ns; k++) {
        if (op_strcmp(candidate,wlst[k]) == 0) cwrd = 0;
      }
      if ((cwrd) && checkword(candidate, wl, cpdsuggest, timer, timelimit)) {
        wlst[ns] = mystrdup(candidate);
        if (wlst[ns] == NULL) {
            for (int j=0; j<ns; j++) op_free(wlst[j]);
            return -1;
        }
        ns++;
      } 
    ...
    int * onlycompoundsug)
{
  ...
  char ** wlst;    
  w_char word_utf[MAXSWL]; // ARRAY OK 2009-02-09 jb
  int wl = 0;

  int nsugorig = nsug;
  char w2[MAXWORDUTF8LEN]; // ARRAY OK 2009-02-09 jb
  const char * word = w;
  int oldSug = 0;
  ...
  if (complexprefixes) {
    ...
    if (*slst) {
        ...
        wlst = (char **) op_malloc(maxSug * sizeof(char *));
...
int SuggestMgr::suggest_auto(char*** slst, const char * w, int nsug)
{
  ...
  char w2[MAXWORDUTF8LEN]; // ARRAY OK 2009-02-09 jb (NOT CHECKED)
  const char * word = w;
  ...
  if (complexprefixes) {
    ...
    if ((nsug < maxSug) && (nsug > -1) && (!cpdsuggest || (nsug < oldSug + maxcpdsugs)) && check_forbidden(word, op_strlen(word))) {
                nsug = twowords(wlst, word, nsug, cpdsuggest);
        }
...
{
  char candidate[MAXSWUTF8L]; // ARRAY OK 2009-02-09 jb
  w_char candidate_utf[MAXSWL]; // ARRAY OK 2009-02-26 jb
  wl = wl > MAXSWL ? MAXSWL : wl;
  op_memcpy(candidate_utf, word, wl * sizeof(w_char));
  mkallcap_utf(candidate_utf, wl, langnum);
  u16_u8(candidate, MAXSWUTF8L, candidate_utf, wl);
  return testsug(wlst, candidate, op_strlen(candidate), ns, cpdsuggest, NULL, NULL);
}

// suggestions for an uppercase word (html -> HTML)
int SuggestMgr::capchars(char** wlst, const char * word, int ns, int cpdsuggest)
{
  char candidate[MAXSWUTF8L]; // ARRAY OK 2009-02-25 jb
  if ( op_strlcpy(candidate, word, sizeof(candidate)) >= sizeof(candidate) )
	  return ns;
...
int SuggestMgr::mapchars(char** wlst, const char * word, int ns, int cpdsuggest)
{
  ...
  timelimit = (clock_t) g_op_time_info->GetRuntimeMS();
  timer = MINTIMER;
    ...
    const mapentry* maptable, int nummap, int * timer, clock_t * timelimit)
{
  if (*(word + wn) == '\0') {
      int cwrd = 1;
      *(candidate + cn) = '\0';
      int wl = op_strlen(candidate);
      for (int m=0; m < ns; m++)
          if (op_strcmp(candidate, wlst[m]) == 0) cwrd = 0;
      if ((cwrd) && checkword(candidate, wl, cpdsuggest, timer, timelimit)) {
          if (ns < maxSug) {
              wlst[ns] = mystrdup(candidate);
  ...
  for (int j = 0; j < nummap; j++) {
    for (int k = 0; k < maptable[j].len; k++) {
      int len = op_strlen(maptable[j].set[k]);
      if (op_strncmp(maptable[j].set[k], word + wn, len) == 0) {
        in_map = 1;
        for (int l = 0; l < maptable[j].len; l++) {
	  op_strcpy(candidate + cn, maptable[j].set[l]);
	  ns = map_related(word, candidate, wn + len, op_strlen(candidate), wlst,
...
int SuggestMgr::replchars(char** wlst, const char * word, int ns, int cpdsuggest)
{
  ...
  for (int i=0; i < numrep; i++ ) {
      r = word;
      lenr = op_strlen(reptable[i].pattern2);
      lenp = op_strlen(reptable[i].pattern);
      // search every occurence of the pattern in the word
      while ((r=op_strstr(r, reptable[i].pattern)) != NULL && (!reptable[i].end || op_strlen(r) == op_strlen(reptable[i].pattern)) &&
        (!reptable[i].start || r == word)) {
          if (r-word + lenr + op_strlen(r+lenp) >= MAXSWUTF8L) break;
          hunspell_strncpy(candidate, word, r-word);
          op_strcpy(candidate+(r-word),reptable[i].pattern2);
          op_strcpy(candidate+(r-word)+lenr, r+lenp);
          ns = testsug(wlst, candidate, wl-lenp+lenr, ns, cpdsuggest, NULL, NULL);
          if (ns == -1) return -1;
          // check REP suggestions with space
          char * sp = op_strchr(candidate, ' ');
          if (sp) {
            char * prev = candidate;
            while (sp) {
              *sp = '\0';
              if (checkword(prev, op_strlen(prev), 0, NULL, NULL)) {
                int oldns = ns;
                *sp = ' ';
                ns = testsug(wlst, sp + 1, op_strlen(sp + 1), ns, cpdsuggest, NULL, NULL);
                if (ns == -1) return -1;
                if (oldns < ns) {
                  op_free(wlst[ns - 1]);
...
int SuggestMgr::doubletwochars(char** wlst, const char * word, int ns, int cpdsuggest)
{
  ...
  int state=0;
  int wl = op_strlen(word);
  if (wl < 5 || wl >= MAXSWUTF8L || ! pAMgr) return ns;
  for (int i=2; i < wl; i++ ) {
      if (word[i]==word[i-2]) {
          state++;
          if (state==3) {
            op_strcpy(candidate,word);
            op_strcpy(candidate+i-1,word+i+1);
...
{
  w_char        candidate_utf[MAXSWL]; // ARRAY OK 2009-02-25 jb
  char          candidate[MAXSWUTF8L]; // ARRAY OK 2009-02-25 jb
  int state=0;
  if (wl < 5 || wl > MAXSWL || !pAMgr) return ns;

  for (int i=2; i < wl; i++) {
      if (w_char_eq(word[i], word[i-2]))  {
          state++;
          if (state==3) {
            op_memcpy(candidate_utf, word, (i - 1) * sizeof(w_char));
            op_memcpy(candidate_utf+i-1, word+i+1, (wl-i-1) * sizeof(w_char));
            u16_u8(candidate, MAXSWUTF8L, candidate_utf, wl-2);
            ns = testsug(wlst, candidate, op_strlen(candidate), ns, cpdsuggest, NULL, NULL);
            if (ns == -1) return -1;
            state=0;
          }
      } else {
...
int SuggestMgr::badcharkey_utf(char ** wlst, const w_char * word, int wl, int ns, int cpdsuggest)
{
  w_char        tmpc;
  w_char        candidate_utf[MAXSWL]; // ARRAY OK 2009-02-26 jb
  char          candidate[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
...
int SuggestMgr::badchar(char ** wlst, const char * word, int ns, int cpdsuggest)
{
  char  tmpc;
  char  candidate[MAXSWUTF8L]; // ARRAY OK 2009-02-25 jb
  clock_t timelimit = (clock_t) g_op_time_info->GetRuntimeMS();
  int timer = MINTIMER;
  int wl = op_strlen(word);
...
int SuggestMgr::badchar_utf(char ** wlst, const w_char * word, int wl, int ns, int cpdsuggest)
{
  ...
  for (int j=0; j < ctryl; j++) {
    for (int i=wl-1; i >= 0; i--) {
       ...
       u16_u8(candidate, MAXSWUTF8L, candidate_utf, wl);
       ns = testsug(wlst, candidate, op_strlen(candidate), ns, cpdsuggest, &timer, &timelimit);
       if (ns == -1) return -1;
       if (!timer) return ns;
       candidate_utf[i] = tmpc;
    }
...
int SuggestMgr::extrachar(char** wlst, const char * word, int ns, int cpdsuggest)
{
   ...
   char *  p;
   int wl = op_strlen(word);
   if (wl < 2 || wl >= MAXSWUTF8L) return ns;
   // try omitting one char of word at a time
   op_strcpy(candidate, word);
...
int SuggestMgr::twowords(char ** wlst, const char * word, int ns, int cpdsuggest)
{
    ...
    int wl=op_strlen(word);
    if (wl < 3 || wl >= MAXSWUTF8L - 1) return ns;
    
    if (langnum == LANG_hu) forbidden = check_forbidden(word, wl);

    op_strcpy(candidate + 1, word);
    ...
    for (p = candidate + 1;  p[1] != '\0';  p++) {
       ...
       *p = '\0';
       c1 = checkword(candidate,op_strlen(candidate), cpdsuggest, NULL, NULL);
       if (c1) {
         c2 = checkword((p+1),op_strlen(p+1), cpdsuggest, NULL, NULL);
         if (c2) {
            *p = ' ';
            ...
            if (ctry && (op_strchr(ctry, 'a') || op_strchr(ctry, '-')) &&
                mystrlen(p + 1) > 1 &&
...
{
   w_char candidate_utf[MAXSWL]; // ARRAY OK 2009-02-09 jb
   char   candidate[MAXSWUTF8L]; // ARRAY OK 2009-02-09 jb
   w_char * p;
   w_char tmpc;
   int len = 0;
   ...
   for (p = candidate_utf;  p < (candidate_utf + wl - 1);  p++) {
      ...
      *p = p[1];
      p[1] = tmpc;
      u16_u8(candidate, MAXSWUTF8L, candidate_utf, wl);
      if (len == 0) len = op_strlen(candidate);
...
int SuggestMgr::ngsuggest(char** wlst, char * w, int ns, HashMgr** pHMgr, int md)
{
  ...
  char w2[MAXWORDUTF8LEN]; // ARRAY OK 2009-02-25 jb
  char fld[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
  char * word = w;
  ...
  char mw[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
  w_char u8[MAXSWL]; // ARRAY OK 2009-02-26 jb
  int n = (utf8) ? u8_u16(u8, MAXSWL, word) : nc;
  
  if ( n >= MAXWORDUTF8LEN || n >= MAXSWUTF8L )
	  return ns;

  // set character based ngram suggestion for words with non-BMP Unicode characters
  if (n == -1) {
	  if ( nc >= MAXWORDUTF8LEN || nc >= MAXSWUTF8L )
		   return ns;
  ...
  char target[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
  char candidate[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
  if (ph) {
    if (utf8) {
      w_char _w[MAXSWL];
    ...
    phonet(candidate, target, nc, *ph, MAXSWUTF8L); // XXX phonet() is 8-bit (nc, not n)
  }
  ...
  while (0 != (hp = (pHMgr[i])->walk_hashtable(col, hp))) {
    ...
    if ((hp->var & H_OPT_PHON) && copy_field(fld, sizeof(fld), HENTRY_DATA(hp), MORPH_PHON)) {
	int sc2 = ngram(3, word, fld, NGRAM_LONGER_WORSE + low) +
		+ leftcommonsubstring(word, fld);
	if (sc2 > sc) sc = sc2;
    }

    scphon = -20000;
    if (ph && (sc > 2) && (op_abs(n - (int) hp->clen) <= 3)) {
      char target2[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
      if (utf8) {
        w_char _w[MAXSWL];
        ...
        op_strlcpy(candidate, HENTRY_WORD(hp), sizeof(candidate));
        mkallcap(candidate, csconv);
      }
      phonet(candidate, target2, -1, *ph, MAXSWUTF8L);
 ...
 // now expand affixes on each of these root words and
  ...
  glst = (struct guessword *) op_calloc(MAX_WORDS,sizeof(struct guessword));
  if (! glst) {
    if (nonbmp) utf8 = 1;
    return ns;
  }
      ...
      if (roots[i]) {
        ...
        int nw = pAMgr->expand_rootword(glst, MAX_WORDS, HENTRY_WORD(rp), rp->blen,
            	    rp->astr, rp->alen, word, nc, 
										((rp->var & H_OPT_PHON) ? copy_field(fld, sizeof(fld), HENTRY_DATA(rp), MORPH_PHON) : NULL));
        ...
        for (int k = 0; k < nw ; k++) {
           ...
           if (sc > thresh) {
              if (sc > gscore[lp]) {
                ...
                op_free(glst[k].word);
                if (glst[k].orig) op_free(glst[k].orig);
              }
           } else {
                op_free(glst[k].word);
                if (glst[k].orig) op_free(glst[k].orig);
  ...
  for (i=0; i < MAX_GUESS; i++) {
      if (guess[i]) {
        // lowering guess[i]
        char gl[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
        int len;
        if (utf8) {
          w_char _w[MAXSWL];
          ...
          u16_u8(gl, MAXSWUTF8L, _w, len);
			...
			op_strlcpy(gl, guess[i], sizeof(gl));
          if (!nonbmp) mkallsmall(gl, csconv);
          len = op_strlen(guess[i]);
        }
...
// phonetic version
      ...
      if (rootsphon[i]) {
        ...
        if (utf8) {
          ...
          u16_u8(gl, MAXSWUTF8L, _w, len);
			...
			op_strlcpy(gl, rootsphon[i], sizeof(gl));
          if (!nonbmp) mkallsmall(gl, csconv);
          len = op_strlen(rootsphon[i]);
        }

        // heuristic weigthing of ngram scores
        scoresphon[i] += 2 * lcslen(word, gl) - op_abs((int) (n - len)) +
          // weight length of the left common substring
          leftcommonsubstring(word, gl);
      }
  }
      ...
      if ((ns < oldns + maxngramsugs) && (ns < maxSug) && (!same || (gscore[i] > 1000))) {
	    ...
	    // keep the best ngram suggestions, unless in ONLYMAXDIFF mode
            if (ns > oldns || (pAMgr && pAMgr->get_onlymaxdiff())) {
    	        op_free(guess[i]);
    	        if (guessorig[i]) op_free(guessorig[i]);
                continue;
            }
        }
        for (j = 0; j < ns; j++) {
          // don't suggest previous suggestions or a previous suggestion with prefixes or affixes
          if ((!guessorig[i] && op_strstr(guess[i], wlst[j])) ||
	     (guessorig[i] && op_strstr(guessorig[i], wlst[j])) ||
            // check forbidden words
            !checkword(guess[i], op_strlen(guess[i]), 0, NULL, NULL)) unique = 0;
        }
        if (unique) {
    	    wlst[ns++] = guess[i];
    	    if (guessorig[i]) {
  ...
  if (ph) for (i=0; i < MAX_ROOTS; i++) {
	...
	int unique = 1;
        for (j = 0; j < ns; j++) {
          // don't suggest previous suggestions or a previous suggestion with prefixes or affixes
          if (op_strstr(rootsphon[i], wlst[j]) || 
            // check forbidden words
            !checkword(rootsphon[i], op_strlen(rootsphon[i]), 0, NULL, NULL)) unique = 0;
        }
        if (unique) {
...
int SuggestMgr::checkword(const char * word, int len, int cpdsuggest, int * timer, clock_t * timelimit)
{
  ...
  if (timer) {
    (*timer)--;
    if (!(*timer) && timelimit) {
      if ((g_op_time_info->GetRuntimeMS() - *timelimit) > TIMELIMIT) return 0;
      *timer = MAXPLUSTIMER;
    }
  }
...
int SuggestMgr::suggest_pos_stems(char*** slst, const char * w, int nsug)
{
  ...
  char w2[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb (NOT CHECKED)
  const char * word = w;
  ...
  if (complexprefixes) {
    ...
    if (*slst) {
        ...
        wlst = (char **) op_calloc(maxSug, sizeof(char *));
        if (wlst == NULL) return -1;
    }

    rv = pAMgr->suffix_check(word, wl, 0, NULL, wlst, maxSug, &nsug);

    // OP_DELETE( dash from end of word)
    if (nsug > 0) {
        for (int j=0; j < nsug; j++) {
            if (wlst[j][op_strlen(wlst[j]) - 1] == '-') wlst[j][op_strlen(wlst[j]) - 1] = '\0';
        }
    }
...
{
    char result[MAXLNLEN];  // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    char * r = (char *) result;
    char * st;
  ...
  char w2[MAXSWUTF8L]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
  const char * word = w;
  ...
  if (complexprefixes) {
    ...
    st = pAMgr->affix_check_morph(word,op_strlen(word));
    if (st) {
    ...
    if (pAMgr->get_compound() && (*result == '\0'))
        pAMgr->compound_check_morph(word, op_strlen(word),
...
{
    char * p = NULL;
    char ** wlst = (char **) op_calloc(maxSug, sizeof(char *));
    if (!**wlst) return NULL;
    ...
    if (ns == maxSug) {
        ...
        op_free(wlst[maxSug - 1]);
    }
    if (wlst) op_free(wlst);
    return p;
}
#endif // END OF HUNSPELL_EXPERIMENTAL CODE
...
char * SuggestMgr::suggest_hentry_gen(hentry * rv, char * pattern)
{
    ...
    *result = '\0';
    ...
    char allomorph[MAXLNLEN]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    char * p = NULL;
    if (HENTRY_DATA(rv)) p = (char *) op_strstr(HENTRY_DATA2(rv), MORPH_ALLOMORPH);
    while (p) {
        struct hentry * rv2 = NULL;
        p += MORPH_TAG_LEN;
        int plen = fieldlen(p);
        hunspell_strncpy(allomorph, p, plen);
        allomorph[plen] = '\0';
        rv2 = pAMgr->lookup(allomorph);
        while (rv2) {
//            if (HENTRY_DATA(rv2) && get_sfxcount(HENTRY_DATA(rv2)) <= sfxcount) {
            if (HENTRY_DATA(rv2)) {
                char * st = (char *) op_strstr(HENTRY_DATA2(rv2), MORPH_STEM);
                if (st && (op_strncmp(st + MORPH_TAG_LEN, 
        ...
        p = op_strstr(p + plen, MORPH_ALLOMORPH);
    }
...
char * SuggestMgr::suggest_gen(char ** desc, int n, char * pattern) {
  char result[MAXLNLEN]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
  char result2[MAXLNLEN]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
  char newpattern[MAXLNLEN]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
  *newpattern = '\0';
  if (n == 0) return 0;
  *result2 = '\0';
  struct hentry * rv = NULL;
...
// search affixed forms with and without derivational suffixes
  ...
  for (int k = 0; k < n; k++) {
    *result = '\0';
    ...
    char **pl;
    char tok[MAXLNLEN]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
    op_strcpy(tok, s);
    ...
    for (int i = 0; i < pln; i++) {
            ...
            char * is = op_strstr(pl[i], MORPH_INFL_SFX);
            if (is) *is = '\0';
            char * ts = op_strstr(pl[i], MORPH_TERM_SFX);
            while (ts) {
                *ts = '_';
                ts = op_strstr(pl[i], MORPH_TERM_SFX);
            }
            char * st = op_strstr(s, MORPH_STEM);
            if (st) {
                copy_field(tok, st, MORPH_STEM);
                rv = pAMgr->lookup(tok);
                while (rv) {
                    char newpat[MAXLNLEN]; // ARRAY OK 2009-02-23 jb (NOT CHECKED)
                    op_strcpy(newpat, pl[i]);
                    op_strcat(newpat, pattern);
                    ...
                    if (sg) {
                        ...
                        for (int j = 0; j < genl; j++) {
                            if (op_strstr(pl[i], MORPH_SURF_PFX)) {
                                int r2l = op_strlen(result2);
                                result2[r2l] = MSEP_REC;
                                op_strcpy(result2 + r2l + 1, result);
                                copy_field(result2 + op_strlen(result2), pl[i], MORPH_SURF_PFX);
  ...
  if (*result2 || !op_strstr(pattern, MORPH_DERI_SFX)) break;
  op_strcpy(newpattern, pattern);
  pattern = newpattern;
  char * ds = op_strstr(pattern, MORPH_DERI_SFX);
  while (ds) {
    hunspell_strncpy(ds, MORPH_TERM_SFX, MORPH_TAG_LEN);
    ds = op_strstr(pattern, MORPH_DERI_SFX);
  }
 }
...
int SuggestMgr::ngram(int n, char * s1, const char * s2, int opt)
{
  ...
  if (utf8) {
    w_char su1[MAXSWL]; // ARRAY OK 2009-02-23 jb
    w_char su2[MAXSWL]; // ARRAY OK 2009-02-23 jb
...
test++;
  ...
  if (opt & NGRAM_ANY_MISMATCH) ns = op_abs(l2-l1)-2;
  ns = (nscore - ((ns > 0) ? ns : 0));
  return ns;
}
...
int SuggestMgr::commoncharacterpositions(char * s1, const char * s2, int * is_swap) {
  int num = 0;
  int diff = 0;
  int diffpos[2]; // ARRAY OK 2009-02-23 jb
  *is_swap = 0;
  if (utf8) {
    w_char su1[MAXSWL]; // ARRAY OK 2009-02-23 jb
    w_char su2[MAXSWL]; // ARRAY OK 2009-02-23 jb
    ...
    int i;
    char t[MAXSWUTF8L]; // ARRAY OK 2009-02-26 jb
    op_strlcpy(t, s2, sizeof(t));
    // decapitalize dictionary word
    if (complexprefixes) {
      int l2 = op_strlen(t);
...
int SuggestMgr::mystrlen(const char * word) {
  if (utf8) {
    w_char w[MAXSWL]; // ARRAY OK 2009-02-23 jb
    return u8_u16(w, MAXSWL, word);
  } else return op_strlen(word);
}
...
void SuggestMgr::lcs(const char * s, const char * s2, int * l1, int * l2, char ** result) {
  int n, m;
  w_char su[MAXSWL]; // ARRAY OK 2009-02-23 jb
  w_char su2[MAXSWL]; // ARRAY OK 2009-02-23 jb
  char * b;
  char * c;
  int i;
  ...
  c = (char *) op_malloc((m + 1) * (n + 1));
  b = (char *) op_malloc((m + 1) * (n + 1));
  if (!c || !b) {
    if (c) op_free(c);
    if (b) op_free(b);
    *result = NULL;
