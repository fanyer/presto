...
#  define CS_ARRAY(type, name) static type name[] = {
#  define CS_ARRAY_DOUBLE_ENTRY(name1, val1, name2, val2) {val1, val2}
#  define CS_ARRAY_TRIPLE_ENTRY(name1, val1, name2, val2, name3, val3) {val1, val2, val3}
#  define CS_ARRAY_END };
#else
#  define CS_ARRAY(type, name) void hunspell_init_##name () { type *local = g_hunspell_##name; int i=0;
#  define CS_ARRAY_DOUBLE_ENTRY(name1, val1, name2, val2) local[i].name1 = val1; local[i++].name2=val2
#  define CS_ARRAY_TRIPLE_ENTRY(name1, val1, name2, val2, name3, val3) local[i].name1 = val1; local[i].name2=val2; local[i++].name3=val3
#  define CS_ARRAY_END ;}
#endif

#define CS_ARRAY_INFO_ENTRY(cc, cl, cu) CS_ARRAY_TRIPLE_ENTRY(ccase, cc, clower, cl, cupper, cu)
#define CS_ARRAY_ENC_ENTRY(e, t) CS_ARRAY_DOUBLE_ENTRY(enc_name, e, cs_table, t)
#define CS_ARRAY_LANG_MAP_ENTRY(l, n) CS_ARRAY_DOUBLE_ENTRY(lang, l, num, n)

#define HUNSPELL_NUM_ENCDS 22
#define HUNSPELL_NUM_LANGS 27
...
#define MORPH_TAG_LEN     op_strlen(MORPH_STEM)
...
LIBHUNSPELL_DLL_EXPORTED char * copy_field(char * dest, size_t size, const char * morph, const char * var);
...
{
    return (HENTRY_DATA(h) ? op_strstr(HENTRY_DATA(h), p) : NULL);
}
