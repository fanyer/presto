...
static const UINT32 alpha_table[256]= 
{
...
static const UINT32 alphainverse_table[256]= 
{
...
static const UINT32 sbox_table_0[256]= 
{
...
static const UINT32 sbox_table_1[256]= 
{
...
static const UINT32 sbox_table_2[256]= 
{
...
static const UINT32 sbox_table_3[256]= 
{
...
#define MultAlpha(x) ((x << 8) ^ alpha_table[x >> 24])

#define MultAlphaInverse(x) ((x >> 8) ^ alphainverse_table[x & 0xff])

#define Sbox(x) (sbox_table_0[x & 0xff] ^ sbox_table_1[(x >> 8) & 0xff] ^ sbox_table_2[(x >> 16) & 0xff] ^ sbox_table_3[x >> 24])   

CryptoStreamCipherSnow *CryptoStreamCipherSnow::CreateSnow()
{
	return OP_NEW(CryptoStreamCipherSnow, ());	
}
 
 

CryptoStreamCipherSnow::CryptoStreamCipherSnow()
: m_key_size(KEY_SIZE_NOT_SET)
, m_sliding_index(0)
, m_R1(0)
, m_R2(0)
, m_counter(0)
...
{
#ifdef VALGRIND
	op_memset(m_inner_state, 0, sizeof(m_inner_state));
#endif
}
...
OP_STATUS CryptoStreamCipherSnow::SetKey(const UINT8 *key, int key_size, const UINT8 *iv)
{	
	if (!key || !iv)
	...
	switch (key_size) {
		case 16:
			m_key_size = KEY_SIZE_128;
			break;

		case 32:
			m_key_size = KEY_SIZE_256;
			break;
	...
	int i, j = 0;
	for (i = (key_size >> 2) - 1; i >= 0; i --)
	{
		m_key[i] = (key[j] << 24) ^ (key[j + 1] << 16) ^ (key[j + 2] << 8) ^ key[j + 3];
		j+=4;
	}
	j = 0;
	for (i = 3; i >= 0; i--)
	{
		m_iv[i] = (iv[j] << 24) ^ (iv[j + 1] << 16) ^ (iv[j + 2] << 8) ^ iv[j + 3];
		j+=4;
	}

	InitiateKey(m_key, m_iv);
	return OpStatus::OK;
}

void CryptoStreamCipherSnow::Encrypt(const UINT8 *source, UINT8 *target, int len)
{
	...
	for (i = 0; i < len; i++)
	{
		...
		target[i] = source[i] ^ ((stream >> (24 - m_counter*8)) & 0xff);
		m_counter = (m_counter + 1)&3;
	}
}

UINT32 CryptoStreamCipherSnow::ClockCipher()
{
	UINT32 prev_R1 = m_R1;
	UINT32 prev_R2 = m_R2;
	m_R1 = m_inner_state[m_sliding_index + 5] + prev_R2;	
	m_R2 = Sbox(prev_R1);

	UINT32 output_stream = ((m_inner_state[m_sliding_index + 15] + prev_R1) ^ prev_R2) ^ m_inner_state[m_sliding_index];

	m_inner_state[m_sliding_index] = m_inner_state[m_sliding_index + 16] = 
		MultAlpha(m_inner_state[m_sliding_index]) ^ m_inner_state[m_sliding_index + 2] ^ MultAlphaInverse(m_inner_state[m_sliding_index + 11]);
	m_sliding_index = (m_sliding_index + 1)&15;	

	return output_stream;
}

void CryptoStreamCipherSnow::RetrieveCipherStream(UINT32 *values, UINT32 length)
{
	for (unsigned i = 0; i < length; i++)
	{
		...
		values[i] = ((m_inner_state[m_sliding_index + 15] + prev_R1) ^ prev_R2) ^ m_inner_state[m_sliding_index];
...
void CryptoStreamCipherSnow::AddEntropy(const UINT8 *entropy_data, int entropy_length)
{
	OP_ASSERT(entropy_data && entropy_length >= 0);
	...
	for (i = 0; i < entropy_length; i++)
	{
		cell <<= 8;
		cell |= entropy_data[i];
		if ((i & 3) == 3 || i == (entropy_length - 1 ))
		{
			m_inner_state[m_sliding_index] ^= cell;
			m_inner_state[m_sliding_index + 5] ^= cell;
			m_inner_state[m_sliding_index + 11] ^= cell;
			m_inner_state[m_sliding_index + 15] ^= cell;
...
void CryptoStreamCipherSnow::ClockCipherFsm()
{
	UINT32 prev_R1 = m_R1;
	UINT32 prev_R2 = m_R2;

	UINT32 fsm = ((m_inner_state[m_sliding_index + 15] + m_R1) ^ m_R2);
	...
	m_inner_state[m_sliding_index] = m_inner_state[m_sliding_index + 16] = 
		MultAlpha(m_inner_state[m_sliding_index]) ^ m_inner_state[m_sliding_index + 2] ^ MultAlphaInverse(m_inner_state[m_sliding_index + 11]) ^ fsm;
	
	m_sliding_index = (m_sliding_index + 1)&0x0f;	
	
}


void CryptoStreamCipherSnow::InitiateKey(UINT32 *key, UINT32 *IV)
{
	OP_ASSERT(key && IV);
	...
	if (m_key_size == KEY_SIZE_128)
	{
		m_inner_state[15] = key[3] ^ IV[0];
		m_inner_state[14] = key[2];
		m_inner_state[13] = key[1];
		m_inner_state[12] = key[0] ^ IV[1];

		m_inner_state[11] = ~key[3];
		m_inner_state[10] = (~key[2]) ^ IV[2];
		m_inner_state[9] = (~key[1]) ^ IV[3];
		m_inner_state[8] = ~key[0];
		
		m_inner_state[7] = key[3];
		m_inner_state[6] = key[2];
		m_inner_state[5] = key[1];
		m_inner_state[4] = key[0];
		                       
		m_inner_state[3] = ~key[3];
		m_inner_state[2] = ~key[2];
		m_inner_state[1] = ~key[1];
		m_inner_state[0] = ~key[0];
	}
	else
	{
		m_inner_state[15] = key[7] ^ IV[0];
		m_inner_state[14] = key[6];
		m_inner_state[13] = key[5];
		m_inner_state[12] = key[4] ^ IV[1];
		...
		m_inner_state[7] = ~key[7];
		m_inner_state[6] = ~key[6];
		m_inner_state[5] = ~key[5];
		m_inner_state[4] = ~key[4];
	...
	for (i = 0; i < 16; i++)
	{
		m_inner_state[i + 16] = m_inner_state[i];
	}
	...
	for (i = 0; i < 32; i++)
	{
		/* Clock the cipher 32 times, and feed the fsm into the lfsr */
		ClockCipherFsm();
	}
	
	/* Clock once, as defined by specification */
	ClockCipher();
}
