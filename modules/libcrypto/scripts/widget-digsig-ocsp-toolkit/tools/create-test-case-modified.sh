...
#author
$toolsdir/sign-widget-modified.sh --pkcs12 "$suitedir/keys/3.rsa.p12" --pwd secret -x -a -i $ID-author -c "$suitedir/keys/2.rsa.cert.pem" $testdir
#with x509 cert
$toolsdir/sign-widget-modified.sh --pkcs12 "$suitedir/keys/3.rsa.p12" --pwd secret -x -i $ID -c "$suitedir/keys/2.rsa.cert.pem" $testdir
$toolsdir/sign-widget-modified.sh --pkcs12 "$suitedir/keys/3.rsa.p12" --pwd secret -i $ID-2 -o "signature2.xml" -c "$suitedir/keys/2.rsa.cert.pem" $testdir
