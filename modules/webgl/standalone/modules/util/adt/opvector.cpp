...
OP_STATUS OpINT32Vector::Add(OpINT32Vector &vec)
{
	void **dst;
	UINT32 i, j, pos, len;
...
OP_STATUS OpINT32Vector::Subtract(OpINT32Vector &vec)
#else
OP_STATUS OpINT32Vector::Substract(OpINT32Vector &vec)
#endif
{
...
OP_STATUS OpINT32Vector::Intersect(OpINT32Vector &vec)
{
	UINT32 i, dpos1, dpos2, dlen, pos, orig_count;
	...
	} while (dpos1 < m_count && i < vec.m_count && (INTPTR)(m_items[dpos1]) == (INTPTR)(vec.m_items[i]));
	...
	for (;;)
	{
		...
		} while (dpos1 + dlen < orig_count && i < vec.m_count &&
			(INTPTR)(m_items[dpos1 + dlen]) != (INTPTR)(vec.m_items[i]));
		...
		} while (dpos2 < orig_count && i < vec.m_count && (INTPTR)(m_items[dpos2]) == (INTPTR)(vec.m_items[i]));
