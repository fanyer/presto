# This file is derived from the Locale::PO module from CPAN.
# Original author: Alan Schwartz, alansz@pennmush.org
# Modifications by: Peter Krefting, peter@opera.com
...
sub load_file_asarray {
    my $self = shift;
    $self->_load_file( $_[0], 0 );
}

sub load_file_ashash {
    my $self = shift;
    $self->_load_file( $_[0], 1 );
}
...
sub _load_file {
    ...
    my $preamble;
    read IN, $preamble, 3; # BOM present?
    if ($preamble ne "\xEF\xBB\xBF")
    {
        seek IN, 0, 0; # No BOM, go back to start of file
    }
    ...
    while (<IN>) {
        ...
        if (/^$/) {
            ...
            if ( defined($po) ) {
              ...
              if (defined $po->{'comment'} && !defined $buffer{'msgstr'} &&
                  !defined $buffer{'msgid'}) {
                  # Block with only translator comments should be discarded
                  warn "Discarding comment-only block";
                  undef $po;
                  next;
              }

              # Check if we have a blank line after just the header
              # (automatic and reference comments without any msgid),
              # if so, just contine on as if we didn't see this blank line.
              unless (defined $po->{'automatic'} && defined $po->{'reference'} &&
                !defined $buffer{'msgid'}) {
                ...
                warn "Discarding stray text block";
                %buffer = ();
                next;
              }
            }
            ...
            # previous string; ignore
        }
        else {
            warn "Strange line in $file line $line_number:\n  \"$_\"\n ";
            if ($_ =~ /[\x0D\x0A]/) {
                die "Check line terminators (need fromdos/todos?)\n"
            }
        }
    }
    if ( defined($po) ) {
...
Opera::PO - Perl module for manipulating .po entries from GNU gettext

=head1 SYNOPSIS

	use Opera::PO;

	$po = new Opera::PO([-option=>value,...])
	...
	$aref = Opera::PO->load_file_asarray(<filename>);
	$href = Opera::PO->load_file_ashash(<filename>);
	Opera::PO->save_file_fromarray(<filename>,$aref);
	Opera::PO->save_file_fromhash(<filename>,$href);
...
interface in which each entry in a .po file is a Opera::PO object.
...
=item new
	...
	my Opera::PO $po = new Opera::PO;
	my Opera::PO $po = new Opera::PO(%options);

Create a new Opera::PO object to represent a po entry.
...
reference to a list of Opera::PO objects corresponding to the contents of
the file, in the same order.

=item load_file_ashash

Given the filename of a po-file, reads the file and returns a
reference to a hash of Opera::PO objects corresponding to the contents of
...
Given a filename and a reference to a list of Opera::PO objects,
saves those objects to the file, creating a po-file.

=item save_file_fromhash

Given a filename and a reference to a hash of Opera::PO objects,
...
I'm hesitant to change this in fear of breaking the modules/scripts of people already using Opera::PO.

Opera::PO requires blank lines between entries, but Uniforum style PO
