...
static int
yaml_parser_fetch_merge(yaml_parser_t *parser);
...
yaml_parser_fetch_next_token(yaml_parser_t *parser)
{
	...
	/* Is it the merge indicator? */

	if (CHECK_AT(parser->buffer, '<', 0)
			&& CHECK_AT(parser->buffer, '<', 1)
		    && CHECK_AT(parser->buffer, ':', 2))
		return yaml_parser_fetch_merge(parser);
...
yaml_parser_fetch_value(yaml_parser_t *parser)
{
    ...
    else
    {
        ...
        if (!parser->flow_level)
        {
			...
			if (parser->simple_key_allowed &&
						!yaml_parser_roll_indent(parser, parser->mark.column, -1,
                        YAML_BLOCK_MAPPING_START_TOKEN, parser->mark))
                return 0;
        }
...
yaml_parser_fetch_merge(yaml_parser_t *parser)
{
	...
	if (!parser->flow_level)
	{
		/* Check if we are allowed to start a new merge (not nessesary simple). */

		if (!parser->simple_key_allowed) {
			return yaml_parser_set_scanner_error(parser, NULL, parser->mark,
					"merges are not allowed in this context");
		}
	...
	/* Simple keys are not allowed after '<<' in the block context. */
	...
	/* Create the MERGE token and append it to the queue. */

	TOKEN_INIT(token, YAML_MERGE_TOKEN, start_mark, end_mark);
            ...
            else if (!single && CHECK(parser->buffer, '\\'))
            {
                ...
                if (code_length)
                {
                    ...
                    if (value <= 0x7F) {
                        ...
                        *(string.pointer++) = 0xC0 + ((value >> 6) & 0x1F);
                        *(string.pointer++) = 0x80 + (value & 0x3F);
                    }
                    else if (value <= 0xFFFF) {
                        *(string.pointer++) = 0xE0 + ((value >> 12) & 0xF);
                        *(string.pointer++) = 0x80 + ((value >> 6) & 0x3F);
                        *(string.pointer++) = 0x80 + (value & 0x3F);
                    }
                    else {
                        *(string.pointer++) = 0xF0 + ((value >> 18) & 0x7);
        ...
        while (IS_BLANK(parser->buffer) || IS_BREAK(parser->buffer))
        {
            if (IS_BLANK(parser->buffer))
            {
                ...
                if (leading_blanks && (int)parser->mark.column < indent
                        && IS_TAB(parser->buffer)) {
                    yaml_parser_set_scanner_error(parser, "while scanning a plain scalar",
                            start_mark, "found a tab character that violates intendation");
                    goto error;
                }
