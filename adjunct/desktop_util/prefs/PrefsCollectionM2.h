...
#if !defined PC_M2_H && defined M2_SUPPORT
#define PC_M2_H

#include "modules/prefs/prefsmanager/opprefscollection_default.h"

/** Global PrefsCollectionM2 object (singleton). */
#define g_pcm2 (g_opera->prefs_module.PrefsCollectionM2())
...
/**
  ...
  */
class PrefsCollectionM2 : public OpPrefsCollectionDefault
{
public:
	/**
	  * Create method.
	  ...
	  */
	static PrefsCollectionM2 *CreateL(PrefsFile *reader);

	virtual ~PrefsCollectionM2();

#include "adjunct/desktop_util/prefs/PrefsCollectionM2_h.inl"
...
# endif
	...
	{
		return PCM2_NUMBEROFSTRINGPREFS + PCM2_NUMBEROFINTEGERPREFS;
	}
...
private:
	/** Single-phase constructor.
	  ...
	  * @param reader Pointer to the PrefsFile object.
	  */
	PrefsCollectionM2(PrefsFile *reader)
		: OpPrefsCollectionDefault(M2, reader)
		{
	...
	PREFS_STATIC struct stringprefdefault
		m_stringprefdefault[PCM2_NUMBEROFSTRINGPREFS + 1];
	/** Integer preference information and default values */
	PREFS_STATIC struct integerprefdefault
		m_integerprefdefault[PCM2_NUMBEROFINTEGERPREFS + 1];
